Процедура записатьВИсториюЗначениеРеквизитаОбъекта(Объект, реквизит, значениеРеквизита, Знач номерВерсии = 0)
	менеджерЗаписи = РегистрыСведений.ИсторияРеквизитовОбъектов.СоздатьМенеджерЗаписи();	
...
функция нужноЗаписыватьИсторию(текущееЗначение, прошлоеЗначение)
	Если ТипЗнч(текущееЗначение) = Тип("ТаблицаЗначений") И ТипЗнч(прошлоеЗначение) = Тип("ТаблицаЗначений") Тогда
		хэшТекущегоЗначения = ОбщийМодуль.получитьХэшMD5(ЗначениеВСтрокуВнутр(текущееЗначение));
		хэшПрошлогоЗначения = ОбщийМодуль.получитьХэшMD5(ЗначениеВСтрокуВнутр(прошлоеЗначение));
...
функция получитьДанныеПоследнейВерсииОбъекта(ссылкаНаОбъект)Запрос = Новый Запрос;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрСведений.ИсторияРеквизитовОбъектов.СрезПоследних(&ДатаПолученияЗначения, Объект = &ОбъектСсылка) КАК ИсторияРеквизитов";
...
Процедура записатьДанныеОбъектаВИсторию(Объект, структураПолейОбъекта) Экспорт
	Если НЕ Константы.ХранитьИсториюРеквизитовОбъектов.Получить() Тогда
		Возврат;
	КонецЕсли;
...
функция получитьЗначениеРеквизитаИзИстории(ссылкаНаОбъект, Реквизит, датаПолученияЗначения) Экспорт
	...
	наборЗаписей = РегистрыСведений.ИсторияРеквизитовОбъектов.ПолучитьПоследнее(датаПолученияЗначения, структураОтбора);
